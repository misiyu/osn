...
class Producer : noncopyable
{
	public:
		...
		void run()
		{
			...
			for (int i = 0; i < prefix_num; i++) {
				...
				cout<<"listen on2 : "<<data_prefix<<endl;
	...
	private:
		...
		void onInterest(const InterestFilter& filter, const Interest& interest)
		{
			...
			string temps = interst_name.substr(0,tmp-2);
			tmp = temps.find_last_of('/');
			string file_name = interst_name.substr( tmp+1 , temps.length()-tmp);
